...
const CategoryCard = ({ item }: { item: (typeof CategoryData)[0] }) => (
  <Link
    href={item.link}
    className="flex flex-col h-full bg-[linear-gradient(150deg,#58C229_30%,#C7DF67_100%)] w-full rounded-lg px-4 py-8 md:px-6 md:py-10 text-center shadow-md hover:shadow-lg hover:-translate-y-3 transition-all duration-400"
  >
    <div className="flex-1 flex flex-col">
      <Image src={item.image} alt="image" width={item.width} height={160} className="mb-4 mx-auto" />
      <h1 className="text-white text-[28px] font-semibold mb-3">{item.title}</h1>
      <p className="text-white text-sm text-[12px] text-justify flex-grow line-clamp-3">{item.desc}</p>
    </div>
    <span className="block w-full h-10 mt-6 rounded-md text-[#AEA46B] bg-white leading-10 font-semibold">Pilih</span>
  </Link>
);
