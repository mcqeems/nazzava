...
const solution = "/image/dirt/solution.webp"
  ...
  return (
    <div className='font-poppins min-h-screen flex flex-col items-center pb-28 lg:px-0 px-6 lg:-mt-12 -mt-26' id='aksi'>
      <div className="flex lg:flex-row flex-col-reverse justify-center items-center lg:gap-64 gap-6">
        ...
        <h1 className='text-transparent bg-clip-text bg-[#5C0000] font-extrabold text-[26px] lg:text-[42px] text-shadow-xl text-center lg:text-right' data-aos="fade-left" data-aos-duration="800">AKSI KECIL, <br /> DAMPAK BESAR</h1>
      </div>
      <h1 data-aos="fade-up" data-aos-duration="800" data-aos-delay="200" className='lg:text-[24px] text-[20px] mt-18 w-fit font-semibold text-white lg:px-14 px-8 py-2 rounded-xl bg-[radial-gradient(circle,#810000_0%,#5C0000_100%)]'>SOLUSI PENANGANAN</h1>
      <div className="mt-15">
        <div className='flex lg:flex-row flex-col lg:items-start items-center justify-center lg:gap-10 gap-8' data-aos-duration="800">
          <div data-aos="fade-up" data-aos-duration="900">
            <Image src={solution} width={500} height={500} alt='solution' className='lg:w-125 w-70 h-auto rounded-2xl transition-transform duration-500 hover:scale-105'/>
          </div>
          <div className=''>
            {DirtAccordion.map((item, i) => (
              <div key={i} className='mt-5' data-aos="fade-up" data-aos-duration="700" data-aos-delay="200">
                <div className={`bg-card w-82.5 lg:w-165 rounded-2xl overflow-hidden transition-all duration-700 bg-gray1 block mx-auto ${activeIndex === i ? 'max-h-125' : 'lg:max-h-16.25 max-h-20'} ${activeIndex !== null ? 'shadow-lg' : ''}`}>
                  <div 
